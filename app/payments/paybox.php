...
if (defined('PAYMENT_NOTIFICATION')) {
    ...
    if (empty($arrRequest['pg_sig']) || !PG_Signature::check(
            $arrRequest['pg_sig'],
            $thisScriptName,
            $arrRequest,
    ...
    if ($mode == 'check') {
        ...
        $arrResponse['pg_sig'] = PG_Signature::make(
            $thisScriptName,
            $arrResponse,
            $processor_data['processor_params']['secret_key']
        );
    ...
    if ($mode == 'result') {
        $bResult = 0;
        if (empty($order_info) || !in_array(
                $order_info['status'],
                array_merge($arrPendingStatuses, $arrOkStatuses, $arrFailedStatuses)
            )) {
            ...
            if ($arrRequest['pg_result'] == 1) {
                ...
                $pp_response['reason_text'] = __('Failed status');
            }
        ...
        // в ответе необходимо указывать тот же pg_salt, что и в запросе
        $objResponse->addChild(
            'pg_salt',
            $arrRequest['pg_salt']
        );
        ...
        $objResponse->addChild(
            'pg_sig',
            PG_Signature::makeXML($thisScriptName, $objResponse, $processor_data['processor_params']['secret_key'])
        );
    ...
    $form_fields = array(
        ...
        'pg_lifetime'       => $processor_data['processor_params']['lifetime'] * 60,
        'pg_testing_mode'   => ($processor_data['processor_params']['test_mode'] == 'test') ? 1 : 0,
        'pg_description'    => mb_substr($strDescription, 0, 255, "UTF-8"),
        'pg_check_url'      => 'http://' . $_SERVER['HTTP_HOST'] . '/index.php?dispatch=payment_notification.check&payment=paybox',
        'pg_result_url'     => 'http://' . $_SERVER['HTTP_HOST'] . '/index.php?dispatch=payment_notification.result&payment=paybox',
        'pg_request_method' => 'GET',
        'pg_success_url'    => 'http://' . $_SERVER['HTTP_HOST'] . '/index.php?dispatch=payment_notification.success&payment=paybox',
        'pg_failure_url'    => 'http://' . $_SERVER['HTTP_HOST'] . '/index.php?dispatch=payment_notification.fail&payment=paybox',
    ...
    $form_fields['pg_sig'] = PG_Signature::make(
        'payment.php',
        $form_fields,
        $processor_data['processor_params']['secret_key']
    );
    ...
    fn_create_payment_form('https://api.paybox.money/payment.php', $form_fields, 'paybox', false);
}
